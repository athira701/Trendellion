...
const addToCart = async (req, res) => {
  try {
    ...
    if (!quantity || !size) {
      return res
        .status(400)
        .json({ message: "Size and quantity are required." });
    }
    ...
    if (existingItemIndex > -1) {
      ...
      cart.item[existingItemIndex].total =
        cart.item[existingItemIndex].price *
...
const deleteCartItem = async (req, res) => {
  try {
    ...
    if (!req.session.user || !req.session.user._id) {
      return res
        .status(401)
        .json({ message: "User not authenticated", redirect: "/login" });
    }
    ...
    const itemToRemove = cart.item.find(
      (item) => item.productId.toString() === productId && item.size === size
    );
    if (!itemToRemove) {
...
const updateCart = async (req, res) => {
  ...
  try {
    ...
    if (!selectedStock) {
      return res
        .status(400)
        .json({ message: "Stock not available for the selected size" });
    }
    ...
    if (newQuantity > MAX_QUANTITY) {
      return res
        .status(400)
        .json({ message: `Maximum quantity allowed is ${MAX_QUANTITY} items` });
    }

    if (newQuantity > selectedStock.quantity) {
      return res
        .status(400)
        .json({ message: `Only ${selectedStock.quantity} items in stock` });
    }
    ...
    // Update cart total
    cart.cartTotal = cart.item.reduce(
